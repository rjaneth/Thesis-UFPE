 ...
 # library(patchwork)
  ...
  for (alpha_val in alpha_values) {
    ...
    plot_bias <- ggplot(df_filtered, aes(x = n, y = Bias, color = Estimator)) +
      ...
      geom_line(linetype = "solid", linewidth = 1.5, alpha = 0.7) +  # Aumenta el grosor de la línea
      labs(y = "Bias", x = expression(italic(n))) +
      ...
      theme(text = element_text(family = "serif"),
            ...
            axis.title = element_text(size = 12),
            plot.title = element_text(size = 11,  hjust = 0),  # Aumenta el tamaño del título de lambda
            legend.position = "bottom",
            legend.box = "horizontal",
            legend.text = element_text(size = 12),  # Aumenta el tamaño del texto de la leyenda
            legend.title = element_text(size = 12)) +  # Aumenta el tamaño del título de la leyenda
      guides(color = guide_legend(nrow = 1))
    
    plot_mse <- ggplot(df_filtered, aes(x = n, y = MSE, color = Estimator)) +
      geom_hline(yintercept = 0, color = "gray50", linetype = "dashed", linewidth = 0.5) +
      geom_point(size = 3.2) +
      geom_line(linetype = "solid", linewidth = 1.5, alpha = 0.7) +
      labs(y = "MSE", x = expression(italic(n))) +
      #scale_x_continuous(breaks = unique(df_filtered$n)) +
      scale_x_continuous(breaks = unique(df_filtered$n), minor_breaks = NULL)+
      scale_y_continuous(minor_breaks = NULL)+
